	...
	numTestData(d)
{
	...
	checkCudaErrors(cudaMalloc((void**)&d_numTestData, sizeof(unsigned int)));
...
int QuadTreeBuilder::allocate()
{
	...
	// allocate device data
	if (d_numTestData==NULL)
	{
		checkCudaErrors(cudaMalloc((void**)&d_numTestData, sizeof(unsigned int)));
		checkCudaErrors(cudaMemset(d_numTestData, 0, sizeof(unsigned int)));
	}
	if (d_left==NULL)
	{
		checkCudaErrors(cudaMalloc((void**)&d_left, sizeof(float)));
...
void QuadTreeBuilder::deallocate()
{
	if (d_numTestData!=NULL)
	{
		checkCudaErrors(cudaFree(d_numTestData));
		d_numTestData = NULL;
	}
	if (d_left!=NULL)
	{
...
{
	checkCudaErrors(cudaMemcpyAsync(d_x, x, dataSz, cudaMemcpyDeviceToDevice, stream));
	checkCudaErrors(cudaMemcpyAsync(d_y, y, dataSz, cudaMemcpyDeviceToDevice, stream));
}
void QuadTreeBuilder::setData(const float* x, const float* y, const float* score, const int d)
{
	checkCudaErrors(cudaMemcpyAsync(d_x, x, d*sizeof(float), cudaMemcpyDeviceToDevice, stream));
	checkCudaErrors(cudaMemcpyAsync(d_y, y, d*sizeof(float), cudaMemcpyDeviceToDevice, stream));
	checkCudaErrors(cudaMemcpyAsync(d_score, score, d*sizeof(float), cudaMemcpyDeviceToDevice, stream));
}
...
int QuadTreeBuilder::filter(float* x, float* y, float* score, unsigned int* d, const int q)
{
	...
	setData(x, y, score, d);
	ResetFilterArrays(q, width, height);
	FilterQuadTreeDev(d_numTestData, q, f);
...
//Async versions
//Takes in device side counter
int QuadTreeBuilder::filter_async(float* x, float* y, float* score, unsigned int* d, const int q)
{
	if (numData < 0)
	{
		fprintf(stderr, "QuadTreeBuilder::filter(): numData < 0, must initialize prior to filtering\n");
...
{
	reset_arrays_kernel<<<blocks, threads, 0, stream>>>(d_mutex, d_x, d_y, d_rx, d_ry, d_child, d_index, d_left, d_right, d_bottom, d_top, w, h, numData, numNodes);
}
void QuadTreeBuilder::ResetFilterArrays(const int q, const int w, const int h)
{
	reset_filter_arrays_kernel<<<blocks, threads, 0, stream>>>(d_mutex, d_x, d_y, d_score, d_xf, d_yf, d_scoref, d_rx, d_ry, d_child, d_index,
...
{
	build_tree_kernel<<<blocks, threads, 0, stream>>>(d_x, d_y, d_rx, d_ry, d_child, d_index, d_left, d_right, d_bottom, d_top, numData, numNodes);
}
...
void QuadTreeBuilder::FilterQuadTree(const int d, const int q, const int f)
{
	...
	filter_tree_kernel<<<blocks, threads, 0, stream>>>(d_x, d_y, d_score, d_rx, d_ry, d_child, d_index, d_left, d_right, d_bottom, d_top, numData, numNodes, d, f);
	...
	pack_filtered_data_kernel<<<blocks, threads, 0, stream>>>(d_xf, d_yf, d_scoref, d_x, d_y, d_score, d_child, numData, d, q);
...
void QuadTreeBuilder::FilterQuadTreeDev(unsigned int* d, const int q, const int f)
{
	...
	filter_treeDev_kernel<<<blocks, threads, 0, stream>>>(d_x, d_y, d_score, d_rx, d_ry, d_child, d_index, d_left, d_right, d_bottom, d_top, numData, numNodes, d, f);
	...
	pack_filteredDev_data_kernel<<<blocks, threads, 0, stream>>>(d_xf, d_yf, d_scoref, d_x, d_y, d_score, d_child, numData, d, q);
